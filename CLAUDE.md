...
- 分類四選一：`parenting`（企鵝寶寶副本）/ `working`（副本裡的大人）/ `travel`（親子行李箱）/ `solo`（一個人的行李箱，副本開啟前、走過七大洲的女孩視角），根據主題判斷；不確定就選最貼近的，不用特地問
...
- Frontmatter 照 `src/content.config.ts` 的 schema：
  ...
  ```
- **pubDate 不能跟現有文章重複**：首頁「最新文章」和「精選」是照 pubDate 新到舊排序，日期相同時排序打平、無法分辨誰才是真正最新。發布前先檢查 `src/content/blog/*.md` 裡有沒有文章已經用掉同一天，若有就往後挪一天，確保每篇文章的 pubDate 都是唯一的。
